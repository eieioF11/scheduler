...
typedef struct
{
    ...
    volatile bool period_enabled; // 【追加】周期実行の有効/無効フラグ
} task_t;
...
void scheduler_init(get_tick_function tick_func)
{
    ...
    for (int i = 0; i < MAX_TASKS; i++)
    {
        ...
        task_list[i].period_enabled = false;
...
bool scheduler_add_task(const char *name, task_function func, void *arg, uint32_t period_ms, task_priority_t priority)
{
    ...
    for (int i = 0; i < MAX_TASKS; i++)
    {
        if (!task_list[i].is_used)
        {
            ...
            task_list[i].period_enabled = true;
            return true;
        }
    }
    return false;
...
void scheduler_enable_period(const char *name) {
    ...
    for (int i = 0; i < MAX_TASKS; i++) {
        if (task_list[i].is_used && strcmp(task_list[i].name, name) == 0) {
            // 無効化されていた期間のズレをリセットするため、現在の時刻を前回実行時刻にする
            if (!task_list[i].period_enabled && get_current_tick != NULL) {
                task_list[i].last_run_tick = get_current_tick();
            }
            task_list[i].period_enabled = true;
            break;
        }
...
void scheduler_update(void)
{
    ...
    for (int p = 0; p < NUM_PRIORITIES; p++)
    {
        for (int i = 0; i < MAX_TASKS; i++)
        {
            ...
            else if (task_list[i].period_enabled && task_list[i].period_ms > 0)
            {
